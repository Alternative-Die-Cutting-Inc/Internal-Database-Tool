...
function echoQuery($category, $field, $searchString) {
    ...
    $query = "SELECT * FROM {$category} WHERE {$field} LIKE '%{$searchString}%' ORDER BY Date DESC Limit {$start}, {$stop}";
    echo $query;
    $result = mysql_query($query, $link);
    ...
    if ($result) {
        ...
        while ($row = mysql_fetch_array($result)) {
            if ($category != "Production") {
                ...
                ."</a></td>". "<td>" . $row['Quote_Number']."</td>"."<td>" . $row['CustomerPoNo']."</td>"."<td>" . $row['Customer']."</td>"."<td>" . $row['ProductionPerson']."</td>"."<td>" . $row['JobName']."</td>"."<td>" . $row['Customer']."</td>"."<td>" . $row['SpecialInstructions']."</td>"."<td>" . $row['Quantity'] ."</td>"."<td>" . $row['FinalPrice']."</td>"."<td>" . $row['Date']."</td>";
                //Action
                echo '<td>
