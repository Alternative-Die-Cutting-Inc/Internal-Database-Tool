    ...
    $http.post(
      ...
      success(function(data, status) {
        if (!data.ERROR){
          ...
          var j = -1;
          for (var i = 1; i <= Object.keys($scope.quoteQuantities).length; i++) {
            if (i % 3 === 1) {
              j++;
              $scope.partition[j] = [];
            }
            $scope.partition[j].push($scope.quoteQuantities[i]);
          }
        } else {
          console.log(data);
        }
  ...
  */
  ...
  $scope.recalculateOnSetAll = function(quantity_id) {
    ...
    for (var key in $scope.quantityExtras[quantity_id]) {
      ...
      var newValue = $scope.quantityExtras[quantity_id][key].cost_per_m;
      $scope.quantityExtras[quantity_id][key].cost = QT.inDollars((100 * newValue * (units/1000))/ 100);
...
QT.updateQuoteQuantityValue = function (quoteNumber, quoteQuantityId, col, val) {
  if (isNaN(val)) {
    ...
    if (col !== "private_notes" && col !== "public_notes") {
      console.log("Attempted to update " + quoteNumber + "." + quoteQuantityId + "." + col + " with " + val);
      return;
    }
  }
  $.ajax({    
