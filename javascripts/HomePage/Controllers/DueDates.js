...
var DueDates = ['$scope', '$http', 'ProductionService', function ($scope, $http, ProductionService) {
	...
	*/
	...
	/*
		Get the jobs by sort by log date and not due date.
	*/
	$scope.sortByLogDate = function() {
		$http.post(
	    "php/DueDatesHelper.php", 
	    { "data" : {
	      request: "get_jobs_by_log_date"
	  		}
	    }).
	    success(function(data, status) {
	    	if (!data.ERROR) {
	    		$scope.filteredJobsByLogDate = data;
	    		console.log(data);
	    	} else {
	    		console.log(data);
	    	}
	...
	*/
	$scope.openProduction = function(docket_number) {
		$scope.currentJob = docket_number;
		$scope.hideShip = true;
